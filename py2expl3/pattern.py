...
def ast_is_leaf(pattern: Pattern)->bool:
	return (
...
def to_pattern_mutable(node: ast.AST)->Pattern:
	scan_node_to_pattern(node)
	return node

Matching=dict[str, ast.AST]

def merge_matching(a: Optional[Matching], b: Optional[Matching])->Optional[Matching]:
...
def pattern_match(pattern: Pattern, tree: ast.AST)->Optional[Matching]:
...
def pattern_replace_mutable(pattern: Pattern, replacement: Matching)->ast.AST:
	"""
